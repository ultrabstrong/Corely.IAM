...
internal partial class Deregistration : CommandBase
{
    ...
    {
        [Argument("Filepath to deregister account request json", true)]
        ...
        public DeregisterAccount(IDeregistrationService deregistrationService)
            : base("account", "Deregister an account")
        {
        ...
        protected override async Task ExecuteAsync()
        {
            if (Create)
            {
                SampleJsonFileHelper.CreateSampleJson(
                    RequestJsonFile,
                    new DeregisterAccountRequest(1)
                );
            }
            else
            {
                await DeregisterAccountAsync();
            }
        }

        private async Task DeregisterAccountAsync()
        {
            var request = SampleJsonFileHelper.ReadRequestJson<DeregisterAccountRequest>(
                RequestJsonFile
            );
            if (request == null)
                return;
