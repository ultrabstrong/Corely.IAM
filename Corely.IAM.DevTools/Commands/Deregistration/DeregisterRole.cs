...
internal partial class Deregistration : CommandBase
{
    internal class DeregisterRole : CommandBase
    {
        ...
        protected override async Task ExecuteAsync()
        {
            if (Create)
            {
                SampleJsonFileHelper.CreateSampleMultipleRequestJson(
                    RequestJsonFile,
                    new DeregisterRoleRequest(Guid.Empty, Guid.Empty)
                );
            }
            else
            {
        ...
        private async Task DeregisterRoleAsync()
        {
            ...
            var request = SampleJsonFileHelper.ReadMultipleRequestJson<DeregisterRoleRequest>(
                RequestJsonFile
            );
            if (request == null)
                return;
            ...
            {
                var result = await _deregistrationService.DeregisterRoleAsync(deregisterRequest);
