...
| `FetchItemsAsync` | `Func<int, int, Task<(List<PickerItem>, int)>>` | none | Server fetch callback `(skip, take)` → `(items, total)` |
| `ExcludeIds` | `IReadOnlyCollection<Guid>` | `[]` | IDs to exclude (already-assigned items) |
| `OnConfirm` | `EventCallback<List<Guid>>` | none | Fired with selected IDs |
...
- Opens via `ShowAsync()`, which resets state and loads the initial batch
